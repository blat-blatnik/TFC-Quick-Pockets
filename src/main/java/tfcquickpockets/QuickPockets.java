...
@Mod(modid=QuickPockets.ID, name=QuickPockets.NAME, version=QuickPockets.VERSION, dependencies=QuickPockets.DEPENDENCIES, guiFactory=QuickPockets.GUI_FACTORY, canBeDeactivated=true)
public class QuickPockets {
    ...
    public void doQuickToolSwap(KeyInputEvent event) {
    ...
    public void cycleThroughInventoryRowsOnMouseScroll(MouseInputEvent event) {
        ...
        if (delta != 0) {
            if (swapHotbar.getIsKeyPressed()) {
                ...
                cyclePlayerInventoryRowsUpOrDown(delta < 0);
            ...
            if (drinkToFix != null && drinkSlotIndexPreUse >= 0 && drinkSlotIndexPostUse >= 0) {
                if (--drinkFixRemainingDelayTicks == 0) {
                    ...
                    if (preDrink == null && postDrink != null && postDrink.getItem() == drinkToFix) {
                        ...
                        swapPlayerInventorySlots(slot1, slot2);
            ...
            while(cycleAgain) {
                ...
                if (cyclingThroughInventory) {
                    if (cycleLastSlotIndex < HOTBAR_SIZE) { // check if we just stated to cycle through inventory.
                        ...
                        swapPlayerInventorySlots(cycleHotbarSlotIndex + 3 * INVENTORY_ROW_SIZE, cycleLastSlotIndex - HOTBAR_SIZE);
                    }
                    ...
                    if (nextIndex < 0) {
                        ...
                        swapPlayerInventorySlots(cycleHotbarSlotIndex + 3 * INVENTORY_ROW_SIZE, nextIndex - HOTBAR_SIZE);
    ...
    public static void cyclePlayerInventoryRowsUpOrDown(boolean up) {
        int inventoryLength = minecraft.thePlayer.inventory.mainInventory.length;
        if (inventoryLength % INVENTORY_ROW_SIZE == 0) { // sanity check - this should always be true
            int numRows = inventoryLength / INVENTORY_ROW_SIZE;
            int numColumns = inventoryLength / numRows;
            for (int column = 0; column < numColumns; ++column) {
                if (up) {
                    for (int row = 0; row < numRows - 1; ++row) {
                        int slot1 = row * INVENTORY_ROW_SIZE + column;
                        int slot2 = (row + 1) * INVENTORY_ROW_SIZE + column;
                        swapPlayerInventorySlots(slot1, slot2);
                    }
                } else {
    ...
    public static void swapPlayerInventorySlots(int slot1Index, int slot2Index) {
        //HACK: I don't even know where to begin with this one....
        //
        // So, you *can't* just set the items in player.inventory by yourself, oh no no no.
        // Because then the new item slots will never get synchronized to the server and bam horrible bug
        // the player will *see* that the items are swapped, but as soon as they try to use one, the server will
        // step in and be like, wait what do you mean you're using a pickaxe - that's a block!
        //
        // I also tried setting the inventory and then sending a S2DPacketSetSlot, which is supposed to be
        // used for syncing stuff like this, but that causes a class cast exception somehow..
        //
        // So in the end I found this function which minecraft calls when the player moves stuff around with the mouse
        // in their inventory - and it actually works! Although of course it's cryptic..
        // So what I do here is I use the first slot of the *crafting area* as a buffer to swap the items. If I don't
        // use it as a buffer, then this would still work - as long as the items in the swapped slots aren't stackable
        // in which case it will not work because the items will stack.
        // Using slots from the crafting area sadly means that this can never 100% work if the player has items in there
        // already, but hey at least it works!

        // UPDATE: I just tried S30PacketWindowItems, but it doesn't work either.

        if (slot1Index != slot2Index) {
            EntityPlayer player = minecraft.thePlayer;
            if (player != null) {
                int slot1 = slot1Index + 9; // everything is offset by 9 since the first 9 slots are crafting grid slots
                int slot2 = slot2Index + 9;
                int inv1 = convertSlotIndexToInventoryIndex(slot1Index);
                int inv2 = convertSlotIndexToInventoryIndex(slot2Index);
                ItemStack[] inventory = player.inventory.mainInventory;
                ItemStack stack1 = inventory[inv1];
                ItemStack stack2 = inventory[inv2];

                // were gonna try to send as few window clicks as possible to avoid lagging servers if players
                // start messing around and cycling hotbars like crazy
                if (stack1 != null || stack2 != null) {
                    if (stack1 == null) {
                        minecraft.playerController.windowClick(0, slot2, 0, 0, player);
                        minecraft.playerController.windowClick(0, slot1, 0, 0, player);
                    } else if (stack2 == null) {
                        minecraft.playerController.windowClick(0, slot1, 0, 0, player);
                        minecraft.playerController.windowClick(0, slot2, 0, 0, player);
                    } else {
                        ...
                        if (item1 != item2) { // these won't stack
                            minecraft.playerController.windowClick(0, slot1, 0, 0, player);
                            minecraft.playerController.windowClick(0, slot2, 0, 0, player);
                            minecraft.playerController.windowClick(0, slot1, 0, 0, player);
                        } else {
                            final int SOME_CRAFTING_SLOT = 1; // DO NOT SET THIS TO 0! 0 is the crafting output slot. 1-4 should be safe.
                            minecraft.playerController.windowClick(0, slot1, 0, 0, player);
                            minecraft.playerController.windowClick(0, SOME_CRAFTING_SLOT, 0, 0, player);
                            minecraft.playerController.windowClick(0, slot2, 0, 0, player);
                            minecraft.playerController.windowClick(0, slot1, 0, 0, player);
                            minecraft.playerController.windowClick(0, SOME_CRAFTING_SLOT, 0, 0, player);
                            minecraft.playerController.windowClick(0, slot2, 0, 0, player);
    ...
    public static void openPlayerInventoryGUI() {
        minecraft.thePlayer.openGui(TerraFirmaCraft.instance, 31, minecraft.theWorld, 0, 0, 0);
    }
        ...
        if (rightClick != 0 && slot != null) {
            if (slot.inventory == minecraft.thePlayer.inventory) { //ASSUMPTION: bags and such can never be inside another bag
                ...
                if (clickedStack != null) {
                    ...
                    if (clickedItem instanceof ItemPotterySmallVessel || clickedItem instanceof ItemQuiver) { //ASSUMPTION: all ItemPotterySmallVessels and ItemQuivers have inventories
                        ...
                        if (player.inventory.getItemStack() == null) {
                            ...
                            if (inventorySlotIndex >= 0 && inventorySlotIndex < HOTBAR_SIZE)
                                ...
                                swapPlayerInventorySlots(playerInventorySlotIndex, hotbarSlotInInventory);
                            }
    ...
    // these were all copy-pasted from com.dunk.tfc.GUI with the added fast bag access fix
    //MAINTENANCE: If any GUIs change this needs to change as well.
    ...
    public static class BasketGUIWithFastBagAccess extends GuiContainerTFC {
        ...
        public static ResourceLocation TEXTURE = new ResourceLocation(ID, "basket-gui-fixed.png");
    ...
    public static class FoodPrepGUIWithFastBagAccess extends GuiContainerTFC {
        ...
        public void initGui() {
            ...
            if (this.guiTab == 0) {
                ...
                buttonList.add((new GuiFoodPrepTabButton(2, this.guiLeft + 36, this.guiTop - 21, 31, 21, this, new ItemStack(TFCItems.salad), TFC_Core.translate("gui.FoodPrep.Salad"))).setButtonCoord(31, 172));
                buttonList.add(new GuiFoodPrepTabButton(1, this.guiLeft + 5, this.guiTop - 21, 31, 21, this, new ItemStack(TFCItems.sandwich), TFC_Core.translate("gui.FoodPrep.Sandwich")));
            } else if (this.guiTab == 1) {
                buttonList.add(new GuiButton(0, this.guiLeft + 74, this.guiTop + 44, 50, 20, TFC_Core.translate("gui.FoodPrep.CreateMeal")));
                buttonList.add(new GuiFoodPrepTabButton(2, this.guiLeft + 36, this.guiTop - 21, 31, 21, this, new ItemStack(TFCItems.salad), TFC_Core.translate("gui.FoodPrep.Salad")));
                buttonList.add((new GuiFoodPrepTabButton(1, this.guiLeft + 5, this.guiTop - 21, 31, 21, this, new ItemStack(TFCItems.sandwich), TFC_Core.translate("gui.FoodPrep.Sandwich"))).setButtonCoord(31, 172));
            }
    ...
    public static class LargeVesselGUIWithFastBagAccess extends GuiContainerTFC {
        public static ResourceLocation TEXTURE = new ResourceLocation(ID, "large-vessel-gui-fixed.png");
    ...
    public static class AnvilGUIWithFastBagAccess extends GuiContainerTFC {
        ...
        public void initGui() {
            super.initGui();
            buttonList.clear();
            buttonList.add(new Button(7, guiLeft + 123, guiTop + 82, 16, 16, TFC_Textures.anvilShrink, 208, 17, 16, 16, this, TFC_Core.translate("gui.Anvil.Shrink")));
            buttonList.add(new Button(6, guiLeft + 105, guiTop + 82, 16, 16, TFC_Textures.anvilUpset, 208, 17, 16, 16, this, TFC_Core.translate("gui.Anvil.Upset")));
            buttonList.add(new Button(5, guiLeft + 123, guiTop + 64, 16, 16, TFC_Textures.anvilBend, 208, 17, 16, 16, this, TFC_Core.translate("gui.Anvil.Bend")));
            buttonList.add(new Button(4, guiLeft + 105, guiTop + 64, 16, 16, TFC_Textures.anvilPunch, 208, 17, 16, 16, this, TFC_Core.translate("gui.Anvil.Punch")));
            buttonList.add(new Button(3, guiLeft + 87, guiTop + 82, 16, 16, TFC_Textures.anvilDraw, 208, 33, 16, 16, this, TFC_Core.translate("gui.Anvil.Draw")));
            buttonList.add(new Button(2, guiLeft + 69, guiTop + 82, 16, 16, TFC_Textures.anvilHitHeavy, 208, 33, 16, 16, this, TFC_Core.translate("gui.Anvil.HeavyHit")));
            buttonList.add(new Button(1, guiLeft + 87, guiTop + 64, 16, 16, TFC_Textures.anvilHitMedium, 208, 33, 16, 16, this, TFC_Core.translate("gui.Anvil.MediumHit")));
            buttonList.add(new Button(0, guiLeft + 69, guiTop + 64, 16, 16, TFC_Textures.anvilHitLight, 208, 33, 16, 16, this, TFC_Core.translate("gui.Anvil.LightHit")));
            buttonList.add(new GuiButton(8, guiLeft + 13, guiTop + 53, 36, 20, TFC_Core.translate("gui.Anvil.Weld")));
            buttonList.add(new Button(9, guiLeft + 113, guiTop + 7, 19, 21, 208, 49, 19, 21, this, 2, TFCOptions.anvilRuleColor2[0], TFCOptions.anvilRuleColor2[1], TFCOptions.anvilRuleColor2[2]));
            buttonList.add(new Button(10, guiLeft + 94, guiTop + 7, 19, 21, 208, 49, 19, 21, this, 1, TFCOptions.anvilRuleColor1[0], TFCOptions.anvilRuleColor1[1], TFCOptions.anvilRuleColor1[2]));
            buttonList.add(new Button(11, guiLeft + 75, guiTop + 7, 19, 21, 208, 49, 19, 21, this, 0, TFCOptions.anvilRuleColor0[0], TFCOptions.anvilRuleColor0[1], TFCOptions.anvilRuleColor0[2]));
            buttonList.add(new PlanButton(12, guiLeft + 122, guiTop + 45, 18, 18, this, TFC_Core.translate("gui.Anvil.Plans")));
        }
